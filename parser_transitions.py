...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        if transition =="S":
            word_new=self.buffer.pop(0)
            self.stack.append(word_new)
        elif transition=="LA":
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        next_trans=model.predict(minibatch)
        for parses,trans in zip(minibatch,next_trans):
            parses.parse_step(trans)
            if len(parses.stack)==1 and len(parses.buffer)==0:
